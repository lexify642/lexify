...
// Thrown *inside* each exported function (not at module load) so it's
// caught by the calling route's own try/catch and returned as a clean
// jsonError(...) instead of an unhandled crash — see lib/supabase.js.
function requireSupabase() {
  if (!supabase) {
    const err = new Error("Cases module is not configured — set SUPABASE_URL and SUPABASE_ANON_KEY in .env (see .env.example).");
    err.code = "CASES_NOT_CONFIGURED";
    throw err;
  }
  return supabase;
}
